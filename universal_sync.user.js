// ==UserScript==
// @name         Universal Web Cache Sync (通用网页缓存同步)
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  一键同步网页的 LocalStorage 和 IndexedDB 缓存至坚果云 WebDAV
// @author       Antigravity
// @match        *://*/*
// @match        file:///*
...
(function() {
    ...
    class WebDAVClient {
        ...
        updateConfig() {
            this.url = GM_getValue("webdav_url", "https://dav.jianguoyun.com/dav/");
            this.user = GM_getValue("webdav_user", "");
            this.pass = GM_getValue("webdav_pass", "");
    ...
    const lastBackupTime = GM_getValue(`last_backup_time_${IDENTIFIER}`, "");
    if (lastBackupTime) {
        valLastSync.innerText = new Date(lastBackupTime).toLocaleString();
    }

    // 填充配置输入框
    inputUrl.value = GM_getValue("webdav_url", "https://dav.jianguoyun.com/dav/");
    inputUser.value = GM_getValue("webdav_user", "");
    inputPass.value = GM_getValue("webdav_pass", "");
    inputDbs.value = GM_getValue(`custom_dbs_${IDENTIFIER}`, "YuyuChatDB, ClothingRecommendDB");
    ...
    function onMouseMove(e) {
        ...
        if (isDragging) {
            ...
            // 面板跟随悬浮球定位
            panel.style.left = `${x - 278}px`;
            panel.style.top = `${y - 390}px`;
    ...
    btnSaveSettings.addEventListener('click', () => {
        GM_setValue("webdav_url", inputUrl.value.trim());
        GM_setValue("webdav_user", inputUser.value.trim());
        GM_setValue("webdav_pass", inputPass.value.trim());
        GM_setValue(`custom_dbs_${IDENTIFIER}`, inputDbs.value.trim());
    ...
    btnUpload.addEventListener('click', async () => {
        ...
        try {
            const customDbsStr = GM_getValue(`custom_dbs_${IDENTIFIER}`, "YuyuChatDB, ClothingRecommendDB");
            ...
            const now = Date.now();
            GM_setValue(`last_backup_time_${IDENTIFIER}`, now);
